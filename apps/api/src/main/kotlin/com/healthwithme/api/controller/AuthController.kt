...
class AuthController(
    ...
    @PostMapping("/google")
    fun googleLogin(@RequestBody request: GoogleLoginRequest): ResponseEntity<ApiResponse<UserDto>> {
        return try {
            val googleUser = googleTokenVerifier.verify(request.idToken)
            val user = userService.loginWithGoogle(googleUser)
            ResponseEntity.ok(
                ApiResponse(success = true, message = "Google login successful", data = user)
            )
        } catch (e: Exception) {
            ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(
